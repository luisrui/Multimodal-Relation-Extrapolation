...
class Trainer(object):
    def __init__(self, args):
        ...
        self.rel2id = json.load(open(self.data_path + "relation2ids.json"))

        # Generate the relation matrix according to word embeddings and TFIDF
        if self.generate_text_embedding:
            if self.dataset == "OpenBG-IMG":
                # OpenBG_text_embedding(self.args, dataname=self.dataset)
                pass
            else:
                raise AttributeError("wrong dataset name!")

        rela_matrix = np.load(self.data_path + "openbg-relations2text.npy")
        print("##LOADING RELATION MATRIX##")
        self.rela_matrix = rela_matrix.astype("float32")

        self.ent2id = json.load(open(self.data_path + "entity2ids.json"))
        ...
        self.Extractor = Extractor(
            self.embed_dim, self.num_symbols, embed=self.symbol2vec
        )
        self.Extractor.cuda()
        ...
        self.Generator = Generator(self.args, input_dim=self.w_dim, ep_dim=self.ep_dim)
        self.Generator.cuda()
        self.Generator.apply(weights_init)
        self.G_parameters = filter(
            lambda p: p.requires_grad, self.Generator.parameters()
        )
    ...
    def load_embed(self):
        ...
        if self.embed_model in ["DistMult", "TransE", "ComplEx", "RESCAL", "TransAE"]:
            ...
            print(
                "    rel_embed shape is {}, the number of relation is {}".format(
                    rel_embed.shape, len(self.rel2id.keys())
                )
            )
            ...
            for key in self.rel2id.keys():
                if key not in ["", "OOV"]:
                    symbol_id[key] = i
                    i += 1
                    embeddings.append(list(rel_embed[int(self.rel2id[key]), :]))

            for key in self.ent2id.keys():
                if key not in ["", "OOV"]:
                    symbol_id[key] = i
                    i += 1
                    embeddings.append(list(ent_embed[int(self.ent2id[key]), :]))

            symbol_id["PAD"] = i
            embeddings.append(list(np.zeros((rel_embed.shape[1],))))
            embeddings = np.array(embeddings)
            np.savez(self.data_path + "Embed_used/" + self.embed_model, embeddings)
            json.dump(
                symbol_id,
                open(self.data_path + "Embed_used/" + self.embed_model + "2id", "w"),
            )
    ...
    def build_connection(self, max_=100):
        ...
        with open(self.data_path + "OpenBG-IMG_train.tsv", 'r') as f:
            lines = f.readlines()
            for line in tqdm(lines):
                e1, rel, e2 = line[:-1].rstrip().split()
    ...
    def load_pretrain(self):
        ...
        self.Extractor.load_state_dict(torch.load('/media/omnisky/sdb/grade2020/cairui/Dawnet/generative-models/WGAN/models/intialExtractor'))
    ...
    def pretrain_Extractor(self):
        ...
        for data in Extractor_generate(
            ...
            query_ep, query_scores = self.Extractor(
                query, support, query_meta, support_meta
            )
            false_ep, false_scores = self.Extractor(
