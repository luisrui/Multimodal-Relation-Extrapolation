...
dataset = 'WN18'
if not os.path.exists('./data/'+dataset+'/img_em'):
    os.mkdir('./data/'+dataset+'/img_em/')
...
batch_dim = 128
count = 0
with open("./data/"+dataset+"/entity2id.txt", "r") as enidf:
    for line in tqdm(enidf.readlines()[1:]):
        entity = line.split('\t')[0]
        #print(entity)
        entity = entity.replace('/', '.')
        input_img = []
        for filename in glob.glob("../origin_data/"+dataset+"/images/n"+entity+"/*.*"):
            #print(filename)
            # try:
        ...
        if len(input_img) > 0 and batch_size + len(input_img) <= batch_dim:
            ...
            for index, item in enumerate(results_split):
                embed = item.mean(0)
                #print(embed.size())
                if not os.path.exists("./data/"+dataset+"/img_em/" + entity_ids[index]):
                    os.mkdir("./data/"+dataset+"/img_em/" + entity_ids[index])
                with open("./data/"+dataset+"/img_em/" + entity_ids[index] + "/avg_embedding.pkl", "wb+") as f:
                    pickle.dump(embed, f)
                    count += 1
