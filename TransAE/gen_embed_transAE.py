...
transe.load_checkpoint("./checkpoints/" + dataset_name + "new.ckpt")

rel_embed = transe.rel_embeddings.weight.data
index = torch.tensor([i for i in range(ents_count)])
ent_embed, loss = transe.ent_embeddings(index)
ent_embed = ent_embed.detach().numpy()

np.savez(dataset_name + "_TransE_embed.npz", rM=rel_embed, eM=ent_embed)
