...
def main(args):
    ...
    triples, mm_info, rel_des_file, e2id, r2id = load_appendix_data(
    ...
    if args.pretrained_model_name != "":
        ...
        model.load_checkpoint(
            f"./saved_models/{args.dataset}/{args.pretrained_model_name}.ckpt",
            device=device,
        )
    ...
    for epoch in epoch_counter:
        ...
        for step, data in enumerate(dataloader):
            ...
            batch_data["rel_des_padding_mask"] = batch_data["rel_des_padding_mask"].to(
                device
            )
            optimizer.zero_grad()
            if len(adjs.edge_index[0]) != 0:
                loss, info = model(
                    local_global_id={
                        k: v.item() for k, v in zip(range(len(n_id)), n_id)
                    },
                ...
                epoch_counter.set_description(
                    "Epoch %d |loss: %.3f |gcn_loss: %.2f |image_loss: %.2f |text_loss: %.2f |con_loss: %.2f "
                    % (
        ...
        if not graph_train_dataset.config.text_only:
            log_image = create_log_images(
                ...
                mean=graph_train_dataset.image_mean,
                std=graph_train_dataset.image_std,
            )
        ...
        if (epoch + args.start_epoch + 1) % args.save_epochs == 0:
            ...
            model.save_checkpoint(
                f"./saved_models/{args.dataset}/epoch{epoch + args.start_epoch + 1}_{args.saved_model_name}.ckpt"
            )
            model.model.set_evaluate(True)
            ent_embs = generate_ent_embed(args, graph_train_dataset, model, device)
            rel_embs = generate_rel_embed(
                graph_train_dataset, model, None, device, "seen"
            )
...
def evaluate(args, ent_embs, rel_embs, e2id, r2id, model, mode="test"):
    ...
    for query in test_candidates.keys():
        ...
        temp_hit_1 = sum([1.0 if rank <= 1 else 0.0 for rank in temp_rank]) / len(
            temp_rank
        )
        temp_hit_3 = sum([1.0 if rank <= 3 else 0.0 for rank in temp_rank]) / len(
            temp_rank
        )
        temp_hit_10 = sum([1.0 if rank <= 10 else 0.0 for rank in temp_rank]) / len(
            temp_rank
        )
        print(
            "Relation: %s| Number %d | mrr: %.4f | hit1: %.4f | hit3: %.4f | hit10: %.4f "
            % (
                query,
...
if __name__ == "__main__":
    ...
    if not args.evaluate:
        ...
        zslmodule.eval(
            generate_model=model.model, mode="test", meta=True, load_pretrain=False
        )
